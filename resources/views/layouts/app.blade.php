...
<nav class="fixed top-0 w-full z-50 bg-black/60 backdrop-blur border-b border-white/10">
    <div class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <div class="space-x-6 text-sm">
             @guest
            <!-- User NOT logged in -->
                <a href="{{ route('login') }}" class="px-5 py-2 rounded-lg bg-transparent border border-white text-white hover:bg-white hover:text-black transition">
                    Login
                </a>
                <a href="{{ route('register') }}"  class="px-5 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition">
                    Register
                </a>
            @endguest
            @auth
                <!-- User logged in -->
                <a href="{{ route('user.dashboard') }}"   class="text-white hover:text-indigo-400">
                    Dashboard
                </a>
            @endauth
