...
<head>
    <title>  <?php echo e(settings('site_name')); ?> | Trade</title>
...
<div class="h-screen ">
    <!-- Header -->
    <div class="shrink-0">
      <?php echo $__env->make('user.trade.header', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    </div>
    <div class="flex-1 flex flex-col md:flex-row gap-4">
        ...
        <div id="chartWrapper" class="flex-1 md:min-h-0   p-4 bg-[#0b0f19] rounded mb-4">
            <div class="flex flex-auto gap-2  mb-3 overflow-x-auto">
                    <?php $__currentLoopData = ['1hr', '6hr', '8hr', '12hr', '1day', '1wk', '2wks', '1month']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $tf): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <button 
                            class="tf-btn bg-[#111827] px-3 py-1 rounded text-sm hover:bg-cyan-500 md:text-sm"
                            data-tf="<?php echo e($tf); ?>"  onclick="setTF('<?php echo e($tf); ?>')">
                            <?php echo e(strtoupper($tf)); ?>
    ...
    <div class="shrink-0 border-t bg-[#0b0f19] md:w-96 p-4 rounded">
        <?php echo $__env->make('user.trade.form', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    </div>
</div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function fetchOpenTrades() {
            fetch(`<?php echo e(route('trade.open')); ?>?mode=${mode}`)
                .then(res => res.json())
                .then(trades => {
        ...
        updateUI(mode);
        fetchUserBalance();
        const hasLiveFunds = <?php echo json_encode($hasLiveFunds ?? false, 15, 512) ?>;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const tradeHistory = <?php echo json_encode($trades->map(function($trade) {
        return [
            'entry_time' => $trade->created_at, 'side' => $trade->side, ];
    })) ?>;;
...
<?php echo $__env->make('user.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Phinehas KwakuFix\Herd\crypto-invest\resources\views/user/trade/index.blade.php ENDPATH**/ ?>